...
class DepositPage:
    def input_deposit_amount(self, amount):
        ...
        amount_input.should(be.visible).should(be.clickable).click()
        amount_input.type(amount)

    def check_deposit_period(self, period):
        browser.element(".finkit-select__icon .private-icon").should(be.visible).should(
            be.clickable
        ).click()
        option = browser.element(
            by.xpath(
                f"//span[@class='finkit-dropdown-option__label' and text()='{period}']"
            )
        )
        option.should(be.visible).should(be.clickable).click()
    ...
    def open_deposit_offer(self):
        browser.all("[data-testid='deposits-item']").should(
    ...
    def check_offer_contains_filter(self, amount, period):
        ...
        browser.element("span._label_2qe4q_137").should(be.visible).should(
            have.text(period)
        )
    ...
    def check_deposit_calculator(self, filters: dict):
        ...
        self.check_offer_contains_filter(
            filters["amount_check"], filters["period_check"]
        )
