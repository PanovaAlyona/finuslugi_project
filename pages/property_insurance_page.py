...
class PropertyPage:
    def verify_fire_chip_in_all_offers(self, filter):
        browser.all('[data-testid="offer-item"]').should(
        ...
        for i, offer in enumerate(offers):
            fire_chips = offer.all(".OfferItem_item__chips__chip__hYGhf").element_by(
                have.exact_text(filter)
            )
    ...
    def filter_offers(self, filter):
        browser.element(".Select__input__c4c9b").with_(timeout=10).should(be.visible)
        browser.element(
            ".Insurance_container__filter__4U5lz .Select__input__c4c9b"
        ).should(be.clickable).click()

        fire_xpath = (
            f'//*[contains(text(), "{filter}")]/ancestor::div'
            '[contains(@class, "option")]'
        )
        fire_option = browser.element(fire_xpath)
        fire_option.should(be.visible).should(be.clickable).click()
